...
### Features
- **Application Load Balancer (ALB)**: We have a brand new [Application Load Balancer](/docs/routing/alb/) available as a backend provider type, with unique and powerful options, including:
  ...
  - General Round Robin
- **Enhanced Health Checking**: [Health checking](/docs/backends/health/) now supports automated health check polling for any backend, and provides a global health status endpoint. Automated health checks for an ALB pool member backend determines whether the ALB will route requests to it or not.
- **Cache Object Chunking**: We now support [Cache Object Chunking](/docs/caching/chunked_caching/). This optional configuration allows a Time Series dataset to be chunked into multiple cache entries based on a configurable chunk size
...
- **Time Series Backend Request Sharding**: We now support [Time Series Backend Request Sharding](/docs/time-series-caching/timeseries_sharding/). This optional feature allows a client request destined for a Time Series Backend to be cloned into multiple concurrent requests with different time ranges constituting the full uncached range
...
- **Authenticator**: We've added a new [Authenticator](/docs/backends/authenticator/) feature so you can guard backends with Basic Auth or ClickHouse Auth
- **Enhanced Rules Engine**: The [Rules Engine](/docs/routing/rule/) now supports `rmatch` operations to permit regular expression-based routing against any part of the HTTP request
...
- **Request Body Size Limit**: A configurable Request Body Size limit has been added for `POST`, `PUT` and `PATCH` requests, with a default of 10MB. Requests with a body size exceeding the limit will receive a `413 Content Too Large` response. See [Request Body Handling Customizations](/docs/request-handling/body/) for more info
...
- **Package Reorganization**: We've re-organized many packages in the codebase to be more easily importable by other projects. A future maintenance release will add documentation to the [examples](https://github.com/trickstercache/trickster/blob/main/examples/) folder for using Trickster packages in your own projects, including caching, acceleration and load balancing
- For Trickster contributors, we have a new [Docker Compose for developer environments](https://github.com/trickstercache/trickster/blob/main/docs/developer/environment/docker-compose.yml).
...
- **Example Configurations**: Example configurations are relocated to the [examples](https://github.com/trickstercache/trickster/blob/main/examples/conf) directory
- **Docker Compose Demo**: The [Trickster docker-compose demo](https://github.com/trickstercache/trickster/blob/main/examples/docker-compose) has been relocated to the examples directory and updated to use the latest version tags. This is the easiest way to try out Trickster 2.0!
