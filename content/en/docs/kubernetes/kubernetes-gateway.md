...
top-level `kubernetes` section ([configuring.md](/docs/getting-started/configuring/)) and grant
it the permissions in [kubernetes-rbac.md](/docs/kubernetes/kubernetes-rbac/). Ingress v1
is served alongside; see [kubernetes-ingress.md](/docs/kubernetes/kubernetes-ingress/).
...
resource; see [kubernetes-cache-policy.md](/docs/kubernetes/kubernetes-cache-policy/).
...
[kubernetes-cache-policy.md](/docs/kubernetes/kubernetes-cache-policy/#targets-and-precedence).
...
runtime; `TCP`, `TLS` and `UDP` open a [stream listener](/docs/getting-started/configuring/#stream-listeners)
...
[tls.md](/docs/listeners/tls/#runtime-certificates) for how a listener holds them and for
...
([kubernetes-deploy.md](/docs/kubernetes/kubernetes-deploy/#ports-and-addresses)); a
...
[paths.md](/docs/request-handling/paths/#path-matching-scope) for the order hosts are tried in.
...
[Mirroring Requests](/docs/request-handling/paths/#mirroring-requests) for what the data plane
...
[Timeouts and Retries](/docs/request-handling/paths/#timeouts-and-retries) for their exact
...
[kubernetes-rbac.md](/docs/kubernetes/kubernetes-rbac/).
...
[kubernetes-rbac.md](/docs/kubernetes/kubernetes-rbac/). Their status is written as an
HTTPRoute's is.
...
when that is unset, a probe of the origin's root every 5 seconds. See [alb-autodiscovery.md](/docs/routing/alb-autodiscovery/)
...
metrics are in [metrics.md](/docs/observability/metrics/). When `kubernetes.defaults.tracing_name`
...
[Graceful shutdown](/docs/getting-started/configuring/#graceful-shutdown-and-readiness)) reports
...
| `proxy_protocol`, `trusted_proxies` | listener | the load balancer's addresses | The real client address in logs and `max_query_range` decisions; see [Trusted Proxies](/docs/getting-started/configuring/#trusted-proxies) |
...
| `access_log` | `kubernetes.defaults` | the `json` preset to stdout | One line per request with the route, upstream and request identifiers; see [access-logs.md](/docs/observability/access-logs/) |
...
through the cache as described in [range_request.md](/docs/object-caching/range_request/).
...
GatewayClass ([kubernetes-deploy.md](/docs/kubernetes/kubernetes-deploy/#ports-and-addresses)).
