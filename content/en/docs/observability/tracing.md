...
- `cache.status` - the lookup or proxy cache status where available. See the [cache status reference](/docs/caching/caches/#cache-status) for a description of the attribute values.
...
- `isPCF` - is true if the origin is configured for [Progressive Collapsed Forwarding](/docs/object-caching/collapsed-forwarding/#progressive-collapsed-forwarding)
