---
title: "Trickster Metrics"
linkTitle: "Metrics"
weight: 10
---

Trickster exposes a Prometheus /metrics endpoint with a customizable listener port number (default is 8481). For more information on customizing the metrics configuration, see [configuring.md](/docs/getting-started/configuring/).
...
* `trickster_proxy_requests_total` (Counter) - The total number of requests Trickster has handled.
  * labels:
    ...
    * `provider` - the type of the configured backend handling the proxy request
    * `method` - the HTTP Method of the proxied request
    * `cache_status` - status codes are described [here](/docs/caching/caches/#cache-status)
    * `http_status` - The HTTP response code provided by the backend
    * `path` - the Path portion of the requested URL
...
* `trickster_alb_pool_admits_failing` (Gauge) - 1 when an ALB pool's `healthy_floor` admits members in the `unavailable` state, 0 otherwise. See [alb.md](/docs/routing/alb/#health-based-backend-selection) for the recommended floor.
  * labels:
    * `backend_name` - the name of the configured ALB backend

* `trickster_alb_pool_floor_reset` (Gauge) - 1 when an ALB pool's `healthy_floor` was reset to 0 at startup because pool members have no health check and could never reach the configured floor, 0 otherwise. See [alb.md](/docs/routing/alb/#health-based-backend-selection).
  * labels:
    * `backend_name` - the name of the configured ALB backend

The following metrics are available when [ALB Autodiscovery](/docs/routing/alb-autodiscovery/) is configured:
...
* `trickster_tls_certificate_expiration_time_seconds` (Gauge) - NotAfter time of a serving TLS certificate, as unix seconds. See [tls.md](/docs/listeners/tls/).
  * labels:
...
The following metrics are available when the Kubernetes Gateway/Ingress controller is enabled (the top-level `kubernetes` section; see [kubernetes-gateway.md](/docs/kubernetes/kubernetes-gateway/)):
