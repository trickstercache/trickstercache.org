...
| `%{cache-status}x` | cache result (`hit`, `phit`, `kmiss`, ...); see [Cache Status](/docs/caching/caches/#cache-status) |
| `%{engine}x` | proxy engine that handled the request (e.g., `DeltaProxyCache`) |
| `%{path-config}x` | the matched [path config](/docs/request-handling/paths/) path |
...
| `%{trace-id}x`, `%{span-id}x` | the request's trace and span identifiers when [tracing](/docs/observability/tracing/) is enabled |
...
is always the connection peer. See [Trusted Proxies](/docs/getting-started/configuring/#trusted-proxies)
