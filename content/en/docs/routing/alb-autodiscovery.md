---
title: "ALB Autodiscovery"
linkTitle: "ALB Autodiscovery"
weight: 20
---

Autodiscovery keeps an [ALB](/docs/routing/alb/) pool's membership current at runtime:
...
the [provider authoring guide](https://github.com/trickstercache/trickster/blob/main/docs/developer/discovery-providers.md).
...
When the ALB uses the [Time Series Merge](/docs/routing/alb/#time-series-merge)
...
[AWS Integration](/docs/backends/aws/). The IAM principal needs
...
Discovered members appear on the [health status page](/docs/backends/health/)
...
[metrics.md](/docs/observability/metrics/). When the ALB has a `tracing_name` configured,
each membership reconcile cycle is traced as an `alb.discovery.reconcile`
span via the standard [tracing](/docs/observability/tracing/) subsystem; the request hot
