...
Per-query behavior can be adjusted with comment directives such as `trickster-backfill-tolerance`; see [Per-Query Instructions](/docs/time-series-caching/per-query-instructions/).
...
Trickster supports enforcing a `max_query_range` limit on ClickHouse backends. For details on how to configure and use query range limits, see the [Query Range Limits](/docs/time-series-caching/query-range-limits/) documentation.
