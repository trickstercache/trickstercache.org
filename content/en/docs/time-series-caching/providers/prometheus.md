...
When using label injection with an ALB configured for [Time Series Merge](/docs/routing/alb/#time-series-merge), injected labels are automatically stripped from responses before merging. This ensures that series from different backends are aggregated correctly, and the injected labels do not appear in the final response to the caller. See the [ALB Merge Strategy documentation](/docs/routing/alb/#merge-strategy) for details.

## Max Query Range Limitation

Trickster supports enforcing a `max_query_range` limit on Prometheus backends. For details on how to configure and use query range limits, see the [Query Range Limits](/docs/time-series-caching/query-range-limits/) documentation.
